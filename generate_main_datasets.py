...
COLUMNORDER_time = ['Bucket', 'Index_time', 'Time',  dim1, dim2, dim3, 'Temperature', 'Pressure', 'Tracer']
...
def average_values_for_time_per_bucket(df):

    df.sort_values(by=['Bucket', 'Index_time'], inplace=True)
    df = df.reset_index(drop=True)

    df_ave = pd.DataFrame()  # all Buckets
    i = 0
    while i <= df['Bucket'].max():
        df_bkt_filt = df.loc[df['Bucket'] == i]  # df of unique xyz
        if len(df_bkt_filt) > 0:
            ...
            while j <= df_bkt_filt['Index_time'].max():
                ...
                if len(df_filtered_) > 0:  # in the Bucket there is an Index_time...
                    xyz_ave['Bucket'] = df_bkt_filt[j:j+1]['Bucket']  # has 1 row ?
                    xyz_ave['Index_time'] = df_bkt_filt[j:j+1]['Index_time']
                    xyz_ave['Time'] = df_bkt_filt[j:j+1]['Time']
                ...
                j += 1
        print("Dset_bkt_TS, average values", i, "/", df['Bucket'].max())
        i += 1
    return df_ave
...
if __name__ == '__main__':
    ...
    Dset_xyz_ave.to_csv('main_datasets_/Dset_xyz_ave_.csv')
    Dset_bkt_TS.to_csv('main_datasets_/Dset_bkt_TS_.csv')
