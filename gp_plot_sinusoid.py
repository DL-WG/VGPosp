...
with tf.name_scope("amplitude"):
    amp_var = tf.Variable(initial_value=[.1], name='amplitude',
                      dtype=np.float64)
...
with tf.name_scope("length_scale"):
    len_var = tf.Variable(initial_value=[0.9], name='length_scale',
                          dtype=np.float64)
...
#####################################
if DO_ASSIGN:
    [_, amplitude_] = sess.run([amplitude_assign,amplitude]  # evaluated in order
                            , feed_dict={amp_p: [0.5]})
...
def plot2D_samples(X, Y): # Plot the true function, observations, and posterior samples.
    ...
    plt.plot(predictive_index_points_, sinusoid(predictive_index_points_),
             label='True fn')
    print("pred_idx_pts[:, 0] : ", predictive_index_points_[:, 0])
    print("obs : ", observations_)
    print("pred_idx_pts : ", predictive_index_points_.shape)
    print("obs : ", observations_.shape)
    ...
    for i in range(X):
        for j in range(Y):
            [_, _, H[i, j]] = sess.run([
                ...
                amplitude_assign,
                log_likelihood
            ], feed_dict={len_p: [2*np.double((1 + i) / X)],
                          amp_p: [2*np.double((1 + j) / Y)]})
    return H
...
plot3D_marginal(200,12,8,'length_scale_assign','amplitude_assign','Z Label')
