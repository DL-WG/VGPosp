...
def normalization_factors_from_training_data(df, FEATURES,
    ...
    saver_mn_stdv = tf.train.Saver({"mean": mean_var, "stdev": stdev_var})
    ...
    with tf.Session() as sess:
        ...
        for i in range(df.shape[0]):
            mean_v, stdev_v, normal_v = sess.run(
                [mean, stdev, normal],
                feed_dict={values: df.loc[i, FEATURES]}
            )
        ...
        save_path = saver_mn_stdv.save(sess, chkpt_name)
...
# extra
import gp_functions as gpf
def calculate_mean_and_stdev(df, FEATURES):
    tf.reset_default_graph()
    values, normal, mean_var, stdev_var = gpf.graph_normalization_factors_from_training_data(df.shape[0], len(FEATURES))

    init_op = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(init_op)
        saver.restore(sess, chkpt_name='normalize/normalize.ckpt')  # loading only affects mean_var and stdev_var (if there are other ops)

        normal_v = sess.run(
            [normal],
            feed_dict={values: df.loc[:, FEATURES]})

        [mean_var_, stdev_var_] = sess.run([mean_var, stdev_var])
        print('mean_var_: ', mean_var_)
        print('stdev_var_: ', stdev_var_)
...
def TEST_normalize():
    ...
    [normal_v] = calculate_mean_and_stdev(df, FEATURES)
